...
export const AGENTS_FAQ: FaqItem[] = [
  {
    question: 'Do I need an API key to let an agent search HireAll?',
    answer:
  ...
  {
    question: 'Which kind of agent works best with HireAll?',
    answer:
      'A local harness — Claude Code, Codex, Cursor, Cline and the like — driving the HireAll CLI. It gets the whole surface, including CV edits and PDF rendering, it can read and write files on your machine, and nothing caps how many tool calls one task may make. Hosted assistants come next: an MCP host such as Claude Desktop runs the same tools over the Model Context Protocol.',
  },
  {
    question: 'How do I connect HireAll to Claude?',
    answer:
      'Two ways. In Claude Code, install the CLI and its agent skills, or add the plugin marketplace. In Claude Desktop, add the HireAll MCP server — it runs via npx, needs no global install, and reuses the CLI credentials if you have already signed in.',
  },
  {
    question: 'Can ChatGPT use HireAll?',
    answer:
      'Yes. There is a published custom GPT wired to the HireAll API, and searching in it needs no setup. To save, apply and track, paste your own API key into the GPT authentication field. You can also import the OpenAPI schema into an Action of your own.',
  },
  {
    question: 'Can my agent apply to jobs for me?',
    answer:
      'No, and that is deliberate. Applications are submitted on the employer’s own site by you; HireAll records that you applied, moves the application through its stages and reads the replies. The browser extension can fill an application form for you to review, but it never submits one on its own.',
  },
  {
